...
dump(CFile *file)
{
  file->write(reinterpret_cast<const uchar *>(&op->ind), sizeof(op->ind));

  file->write(static_cast<const uchar *>(&num_values1), sizeof(num_values1));

  for (uint i = 0; i < num_values1; ++i)
    file->write(reinterpret_cast<const uchar *>(&values1[i]), sizeof(values1[i]));

  file->write(reinterpret_cast<const uchar *>(&num_values2), sizeof(num_values2));

  for (uint i = 0; i < num_values2; ++i)
    file->write(reinterpret_cast<const uchar *>(&values2[i]), sizeof(values2[i]));
}
...
undump(CFile *file)
{
  ...
  file->read(reinterpret_cast<uchar *>(&s), sizeof(s));
  ...
  file->read(reinterpret_cast<uchar *>(&b), sizeof(b));
...
getArgString(ushort pc, uint type, uint arg, uchar *args, ushort)
{
  ...
  if      (type == A_NUM) {
    if      (arg == 1)
      ...
      ushort s = ushort((args[1] << 8) | args[0]);
    ...
    if      (arg == 1) {
      schar o = *reinterpret_cast<schar *>(&args[0]);
